...
import { useEditStudentCertificateMutation, useGetStudentCertificateByIdQuery } from '../../../redux/api/api';
...
const EditStudentCertificate = ({route}) => {
  ...
  const {data, isLoading, error,refetch} = useGetStudentCertificateByIdQuery(id);
  const [editStudentCertificate] = useEditStudentCertificateMutation();
  ...
  const handleSubmission = async (values, {setSubmitting}) => {
    ...
    try {
      ...
      const res = await editStudentCertificate(experienceToSubmit);
