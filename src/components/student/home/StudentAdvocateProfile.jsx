...
import StudentAdvocateReview from '../review/StudentAdvocateReview';

const StudentAdvocateProfile = ({navigation, route}) => {
  ...
  const {data, isLoading, error, refetch} =
    useGetStudentAdvocateByIdQuery(advocateId);
  ...
  const [studentFollowRequest] = useStudentFollowRequestMutation();
  const [studentPrivateChat] = useStudentPrivateChatMutation();
  ...
  const handleFeedback = () => {
    navigation.navigate('FeedbackAdvocate', {id: advocateId});
  };

  const handleConnection = async advocateId => {
    try {
      const res = await studentSendRequest({userId: advocateId}).unwrap();
      console.log(res);
      if (res && res?.success) {
        try {
  ...
  const handleFollow = async advocateId => {
    try {
      const res = await studentFollowRequest({followee: advocateId}).unwrap();
      // console.log(res);
      if (res && res?.success) {
        showMessage({
          message: 'Success',
  ...
  const handleMessage = async advocate => {
    ...
    try {
      const res = await studentPrivateChat({member: advocate._id}).unwrap();
      console.log(res);
          ...
          <View style={{marginTop: hp(2)}}>
            <LinearGradient
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  navigation.navigate('AdvocateBooking', {id: advocateDetails._id})
                }>
                <Image
...
{advocateDetails?.headLine &&  <View style={{marginTop: hp(0.7), paddingHorizontal: wp(1)}}>
            <Text
              style={{
                color: '#294776',
        ...
        {/* <View
          ...
          <TouchableOpacity
            onPress={() => navigation.navigate('StudentAdvocateFollower', {id: advocateId})}>
          ...
          <View>
            <TouchableOpacity
              onPress={() =>
                navigation.navigate('StudentAdvocateFollowing', {id: advocateId})
              }>
              <Text style={{fontFamily: 'Poppins', alignSelf: 'center'}}>
            ...
            <View style={styles.detailsContainer}>
              ...
              <View
                ...
                <View style={styles.detailItem}>
                  ...
                  <Text style={styles.detailValue}>
                  {(advocateDetails && advocateDetails?.specialization[0]?.name) ||
                        'N/A'}
                  </Text>
                </View>
                <View style={styles.detailItem}>
        ...
        {activeTab === 'Review' && (
          <View style={{marginTop: 10}}>
            <StudentAdvocateReview id={advocateId} />
          </View>
        )}
      </View>
      {/* <TouchableOpacity
...
export default StudentAdvocateProfile;
