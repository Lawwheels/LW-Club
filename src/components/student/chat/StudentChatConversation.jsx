...
const StudentChatConversation = ({navigation, route}) => {
  ...
  const {data, isLoading, error, refetch} = useGetStudentChatByIdQuery({
    id: _id, // Chat ID in params
    page: page,
  });

  const [studentChatImage] = useStudentChatImageMutation();
  ...
  const handleAttachmentPress = async () => {
    ...
    if (!result.didCancel && result.assets) {
      ...
      try {
        ...
        const response = await studentChatImage(formData).unwrap();
  ...
  // Handle error state
  if (error) {
    console.log('studentChatConversation', error);
    handleError(error);
