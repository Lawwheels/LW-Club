...
import {
  useGetStudentFollowerQuery,
  useStudentPrivateChatMutation,
} from '../../../redux/api/api';
import {handleError} from '../../../../shared/authUtils';
import {useNavigation} from '@react-navigation/native';

const StudentFollower = ({route}) => {
  const {id} = route.params || {};
  const navigation = useNavigation();
  const [refresh, setRefresh] = useState(false);
  const {data: follow, loading, error} = useGetStudentFollowerQuery(id);
  const [studentPrivateChat] = useStudentPrivateChatMutation();
  //   console.log('follow', follow);
  const following = follow?.data;
  console.log('followingstudent', following);

  const handleMessage = async id => {
    try {
      const res = await studentPrivateChat({member: id}).unwrap();
      // console.log(res);
